...
async function render(){
 ...
 $("spins").textContent=h.length;$("spinCounter").textContent=h.length+" spins";$("predCount").textContent=ps.filter(x=>x.prediction).length;$("evaluated").textContent=bt.n;$("history").innerHTML=h.slice(-24).map(n=>"<span class='"+col(n)+"'>"+n+"</span>").join("");
