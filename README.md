...
## v9.0
- Replaced IndexedDB as the runtime persistence layer with localStorage to eliminate the DB ERROR blocker on mobile/GitHub Pages.
- Number buttons are created before storage initialization.
- Spins are persisted immediately and the counter updates immediately.
- Existing v9 storage is independent from previous failed runtime state.
- The previous IndexedDB is not deleted; a JSON backup should be used if old data needs to be recovered.
