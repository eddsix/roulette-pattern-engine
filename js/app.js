...
(()=>{
'use strict';
const VERSION='15.8.0', KEY='roulettePatternLab.v15.2', THEME_KEY='roulettePatternLab.theme';
...
function familyDistribution(h,f){
  const key=hashHistory(h,0)+'|familydist15.8|'+f;
...
function walkForwardRows(h,f,tol,start=Math.max(12,h.length-160),end=h.length){
  const key=hashHistory(h,tol)+'|wf15.8|'+f+'|'+start+'|'+end;if(cache.walkForward?.has(key))return cache.walkForward.get(key);
...
function metaBacktest(h,tol){
  const key=hashHistory(h,tol)+'|meta15.8';if(cache.meta.has(key))return cache.meta.get(key);const combos={};
...
function adaptive(h,tol){
  ...
  const alpha=0.22;
  FAMILIES.forEach(f=>{
    let w=0.55,seen=false;const rows=walkForwardRows(h,f,tol,12,h.length);
    for(let k=0;k<rows.length;k++){
      const windowRows=rows.slice(Math.max(0,k-159),k+1),n=windowRows.length,hit=windowRows.filter(x=>x.hit).length,recentN=Math.min(20,n),recentRows=windowRows.slice(-recentN),recentHit=recentRows.filter(x=>x.hit).length,mid=Math.max(10,Math.floor(n/2)),first=windowRows.slice(0,mid),second=windowRows.slice(mid),firstHit=first.filter(x=>x.hit).length,secondHit=second.filter(x=>x.hit).length;
      const edge=hit/n-baseline(tol),recentEdge=recentHit/recentN-baseline(tol),robust=robustEdgeWilson(hit,n,tol),ra=firstHit/mid,rb=second.length?secondHit/second.length:0,stability20=clamp(1-Math.abs(edge-recentEdge)/Math.max(0.05,Math.abs(edge)+0.02),0,1),stabilityHalf=clamp(1-Math.abs(ra-rb)/Math.max(0.10,Math.abs(ra)+Math.abs(rb)+0.10),0,1),p={n,hit,edge,recentEdge,robustEdge:robust,stability:0.55*stability20+0.45*stabilityHalf};
...
function model(h,tol){
  ...
  const dirDist=directionDistribution(h),dirPattern=directionPatternDistribution(h),score=Array(37).fill(0),support=Array(37).fill(0),contrib={};
  const active=FAMILIES.filter(f=>familyScores[f].some(v=>v>0)).length;
  if(active<2){cache.model.set(key,null);return null}
  const familyWeightSum=FAMILIES.reduce((s,f)=>s+(a.weights[f]||0),0)||1;
  for(const f of FAMILIES){
    const d=familyScores[f],w=(a.weights[f]||0.55)/familyWeightSum*FAMILIES.length,perf=a.perf[f],sample=perf.n?clamp(Math.sqrt(perf.n/(perf.n+18)),0.30,1):0.30,quality=clamp(0.78+perf.robustEdge*3.5+perf.recentEdge*1.5,0.60,1.25),fw=w*sample*quality;
    contrib[f]=Array(37).fill(0);
    for(let n=0;n<37;n++){
      const evidence=Math.max(0.000001,d[n]*37),term=Math.log1p(fw*Math.max(0,evidence-1));score[n]+=term;contrib[f][n]=term;support[n]+=term;
    }
  }
  // Direction is a contextual feature, not a standalone predictor. It modifies the
  // ensemble according to how compatible each candidate is with the current direction pattern.
  for(let n=0;n<37;n++){
    const dirEvidence=Math.max(0.000001,(0.62*dirDist[n]+0.38*dirPattern[n])*37);
    score[n]+=0.42*Math.log1p(Math.max(0,dirEvidence-1));
  }
  // Explicit interactions: candidates supported by different dimensions together
  // receive a modest bonus; a single historical successor cannot create this bonus.
  ...
  pairMix.forEach(([fa,fb])=>{
    ...
    for(let n=0;n<37;n++){
      ...
      const agreement=Math.sqrt(Math.max(0,aev-1)*Math.max(0,bev-1));
      score[n]+=0.075*Math.log1p(agreement);
    }
  });
  // Current jump + direction compatibility. This uses the current state rather than
  // copying a historical successor number.
  const last=h.at(-1);let cw=0,ccw=0;for(let n=0;n<37;n++){const j=jmp(last,n);if(j>0)cw+=dirDist[n];else if(j<0)ccw+=dirDist[n]}
  const dirSign=cw>=ccw?1:-1;
  for(let n=0;n<37;n++){const j=jmp(last,n);if(j!==0&&Math.sign(j)===dirSign)score[n]+=0.055*Math.min(2,Math.abs(j)/6)}
  ...
  const total=score.reduce((a,b)=>a+b,0)||1;const raw=score.map(v=>v/total),probs=calibrateRelative(raw),ranked=raw.map((v,n)=>({n,v,p:probs[n],support:support[n]})).sort((x,y)=>y.p-x.p||y.support-x.support),top=ranked[0],second=ranked[1],lead=top.p-second.p;
  const mean=1/37,topLift=top.p/mean;
  if(!top||top.p<0.05||topLift<1.18){cache.model.set(key,null);return null}
  ...
  const target=top.n,jump=jmp(last,target),familyContrib=FAMILIES.map(f=>({family:f,score:contrib[f][target]||0})).sort((a,b)=>b.score-a.score);
  const result={target,prob:top.p,zoneProbability,ranking:ranked.slice(0,12),predDir:cw>=ccw?'CW':'CCW',cw,ccw,adaptive:a,seq:candidates(h,'sequence').slice(0,4),joint:candidates(h,'joint').slice(0,3),jumps:candidates(h,'jump').slice(0,3),edge,robustEdge:avgRobust,confidence,signal,lead,avgEdge:avgRobust,avgRecent,stability,consensusCount,activeModels:active,jump,quality,familyTargets,directionContext:directionContext(h),directionPattern:dirPattern,familyContributions:familyContrib,agreementCount,coverage:topLift};
