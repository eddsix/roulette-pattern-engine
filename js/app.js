...
function drawChart(){
  ...
  x.font='600 10px system-ui';x.textAlign='left';x.fillStyle=muted;x.fillText('Baseline '+base.toFixed(1)+'%',pad.l+7,Math.max(12,by-8));
  ...
  x.font='600 10px system-ui';x.textAlign='left';x.fillStyle=muted;x.fillText('Sin señal',w-pad.r-55,18);
  const edge=(evaluated.length?100*hits/evalCount:base)-base;x.fillStyle=edge>=0?accent:muted;x.textAlign='right';x.fillText('Edge '+(edge>=0?'+':'')+edge.toFixed(1)+' pp',w-pad.r,18);
  ...
  if(ev&&pts.length){const rect=c.getBoundingClientRect(),mx=ev.clientX-rect.left;let nearest=pts[0];pts.forEach(p=>{if(Math.abs(p.x-mx)<Math.abs(nearest.x-mx))nearest=p});if(Math.abs(nearest.x-mx)<20){
    const r=nearest.row,tipW=205,tipH=nearest.eval?105:82,tx=Math.min(Math.max(8,nearest.x-tipW/2),w-tipW-8),ty=Math.max(8,(nearest.y||y(base))-tipH-14);x.fillStyle=styles.getPropertyValue('--card').trim()||'#fff';x.strokeStyle=line;x.lineWidth=1;x.beginPath();x.roundRect(tx,ty,tipW,tipH,10);x.fill();x.stroke();x.fillStyle=text;x.textAlign='left';x.font='600 11px system-ui';x.fillText('#'+r.spinIndex+(nearest.eval?'  '+(dist(r.prediction.target,r.actual)<=tol?'ACIERTO':'FALLO'):'  SIN SEÑAL'),tx+10,ty+18);x.font='10px system-ui';x.fillStyle=muted;if(nearest.eval){x.fillText('Predicción: '+r.prediction.target,tx+10,ty+38);x.fillText('Resultado: '+r.actual,tx+10,ty+54);x.fillText('Distancia: '+dist(r.prediction.target,r.actual)+'   ±'+tol,tx+10,ty+70);x.fillText('Rendimiento: '+nearest.v.toFixed(1)+'%',tx+10,ty+88);}else{x.fillText('Resultado: '+r.actual,tx+10,ty+40);x.fillText('No había señal (score < 5%)',tx+10,ty+58);x.fillText('No contabilizado como evaluación',tx+10,ty+74);}
  }}
}
