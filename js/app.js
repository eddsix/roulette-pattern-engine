...
(()=>{
'use strict';
const VERSION='15.9.0', KEY='roulettePatternLab.v15.2', THEME_KEY='roulettePatternLab.theme';
...
function familyDistribution(h,f){
  const key=hashHistory(h,0)+'|familydist15.9|'+f;
  cache.familyDistribution??=new Map();
  if(cache.familyDistribution.has(key))return cache.familyDistribution.get(key);
  const out=Array(37).fill(0),last=h.at(-1),N=h.length;
  if(N<12||last==null){cache.familyDistribution.set(key,out);return out}
  const tr=getTrans(h), jumps=tr.map(x=>x.j), dirs=tr.map(x=>x.d);
  const evidence=[];
  const pushEvidence=(targets,weight,len,occ,lastIndex,sigma)=>{
    if(!targets?.length)return;
    evidence.push({targets,weight:Math.max(0,weight),len,occ,lastIndex,sigma});
  };
  const recency=(i)=>i>=0?0.55+0.45*((i+1)/Math.max(1,N)):0.55;
  if(f==='sequence'){
    for(let l=2;l<=8&&l<N;l++){
      const sig=h.slice(-l).join(','),matches=[];
      for(let i=0;i+l<N;i++)if(h.slice(i,i+l).join(',')===sig)matches.push({value:h[i+l],index:i+l});
      if(matches.length)pushEvidence(matches.map(x=>x.value),1.0+0.12*l, l, matches.length, matches.at(-1).index, l>=6?2.0:2.5);
    }
  }else if(f==='pair'){
    if(N>=3){const a=h.at(-2),b=h.at(-1),m=[];for(let i=0;i+2<N;i++)if(h[i]===a&&h[i+1]===b)m.push({value:h[i+2],index:i+2});if(m.length)pushEvidence(m.map(x=>x.value),1.5,2,m.length,m.at(-1).index,2.5)}
  }else if(f==='transition'){
    const m=[];for(let i=0;i<N-1;i++)if(h[i]===last)m.push({value:h[i+1],index:i+1});if(m.length)pushEvidence(m.map(x=>x.value),1.35,1,m.length,m.at(-1).index,2.7);
  }else if(f==='jump'){
    for(let l=2;l<=6&&l<jumps.length;l++){
      const sig=jumps.slice(-l).join(','),m=[];
      for(let i=0;i+l<jumps.length;i++)if(jumps.slice(i,i+l).join(',')===sig)m.push({value:jumps[i+l],index:i+l});
      if(m.length)pushEvidence(m.map(x=>jumpDestination(last,x.value)),1.05+0.14*l,l,m.length,m.at(-1).index,l>=5?1.55:2.1);
    }
  }else if(f==='joint'){
    for(let l=2;l<=6&&l<jumps.length;l++){
      const sig=jumps.slice(-l).map((j,i)=>j+':'+dirs[jumps.length-l+i]).join('|'),m=[];
      for(let i=0;i+l<jumps.length;i++){
        const test=jumps.slice(i,i+l).map((j,k)=>j+':'+dirs[i+k]).join('|');
        if(test===sig)m.push({value:jumps[i+l],index:i+l});
      }
      if(m.length)pushEvidence(m.map(x=>jumpDestination(last,x.value)),1.15+0.15*l,l,m.length,m.at(-1).index,l>=5?1.5:2.0);
    }
  }
  evidence.forEach(q=>{
    const lenBoost=1+Math.min(0.35,Math.max(0,q.len-2)*0.09);
    const occBoost=1+0.42*Math.log1p(q.occ);
    const rec=recency(q.lastIndex);
    const reliability=(q.occ>=2?1:0.45)*(q.len>=3?1.05:0.9);
    const w=q.weight*lenBoost*occBoost*rec*reliability;
    const counts=new Map();q.targets.forEach(n=>counts.set(n,(counts.get(n)||0)+1));
    const total=q.targets.length||1;
    counts.forEach((count,n)=>addKernel(out,n,w*(count/total),q.sigma));
  });
  const z=out.reduce((a,b)=>a+b,0)||1,result=out.map(v=>v/z);
  cache.familyDistribution.set(key,result);if(cache.familyDistribution.size>CACHE_LIMIT)cache.familyDistribution.delete(cache.familyDistribution.keys().next().value);
  return result;
}
function directionDistribution(h){
  const out=Array(37).fill(0),last=h.at(-1);if(last==null||h.length<2)return out;
  const tr=getTrans(h),recent=tr.slice(-Math.min(24,tr.length));
  let cw=1,ccw=1,same=.5;
  recent.forEach((x,k)=>{const w=.45+.55*((k+1)/recent.length);if(x.d==='CW')cw+=w;else if(x.d==='CCW')ccw+=w;else same+=w*.5});
  const z=cw+ccw+same,pcw=cw/z,pccw=ccw/z,ps=same/z;
  for(let n=0;n<37;n++){const j=jmp(last,n);out[n]=j===0?ps/Math.max(1,37):j>0?pcw/18:pccw/18}
  const s=out.reduce((a,b)=>a+b,0)||1;return out.map(v=>v/s);
}
function directionPatternDistribution(h){
  const out=Array(37).fill(0),tr=getTrans(h),dirs=tr.map(x=>x.d),last=h.at(-1);if(dirs.length<2||last==null)return out;
  for(let l=2;l<=6&&l<dirs.length;l++){
    const sig=dirs.slice(-l).join('|'),m=[];
    for(let i=0;i+l<dirs.length;i++)if(dirs.slice(i,i+l).join('|')===sig)m.push({d:dirs[i+l],j:tr[i+l]?.j,index:i+l});
    m.forEach((q,k)=>{if(q.j==null)return;addKernel(out,jumpDestination(last,q.j),(.9+0.12*l)*(0.65+0.35*((q.index+1)/dirs.length))/(1+k*.05),2.0)});
  }
  const z=out.reduce((a,b)=>a+b,0)||1;return out.map(v=>v/z);
}
function directionContext(h){
  const d=directionDistribution(h),p=directionPatternDistribution(h),out=Array(37).fill(0);
  for(let n=0;n<37;n++)out[n]=0.55*d[n]+0.45*p[n];
  const z=out.reduce((a,b)=>a+b,0)||1;return out.map(v=>v/z);
}
...
function walkForwardRows(h,f,tol,start=Math.max(12,h.length-160),end=h.length){
  const key=hashHistory(h,tol)+'|wf15.9|'+f+'|'+start+'|'+end;if(cache.walkForward?.has(key))return cache.walkForward.get(key);
...
function metaBacktest(h,tol){
  const key=hashHistory(h,tol)+'|meta15.9';if(cache.meta.has(key))return cache.meta.get(key);const combos={};
...
function adaptive(h,tol){
  ...
  // Performance and uncertainty are deliberately separated. Observed edge is the
  // main signal; robust edge is only a confidence modifier, so small samples do not
  // collapse a family to the minimum weight.
  const targetWeight=p=>{
    if(!p||!p.n)return 1;
    const sampleConf=p.n/(p.n+50);
    const observedSignal=(0.65*p.edge+0.25*p.recentEdge+0.10*p.robustEdge)/0.05;
    const stabilityFactor=0.75+0.50*clamp(p.stability,0,1);
    const uncertaintyFactor=clamp(1+1.25*p.robustEdge,0.88,1.12);
    const adjusted=observedSignal*sampleConf*stabilityFactor;
    return clamp(1+0.30*adjusted*uncertaintyFactor,0.75,1.35);
  };
  const alpha=0.22;
  FAMILIES.forEach(f=>{
    let w=1,seen=false;const rows=walkForwardRows(h,f,tol,12,h.length);
    for(let k=0;k<rows.length;k++){
      const windowRows=rows.slice(Math.max(0,k-159),k+1),n=windowRows.length;
      const recentN=Math.min(20,n),recentRows=windowRows.slice(-recentN);
      const hit=windowRows.filter(x=>x.hit).length,recentHit=recentRows.filter(x=>x.hit).length;
      const mid=Math.max(10,Math.floor(n/2)),first=windowRows.slice(0,mid),second=windowRows.slice(mid);
      const firstHit=first.filter(x=>x.hit).length,secondHit=second.filter(x=>x.hit).length;
      const edge=hit/n-baseline(tol),recentEdge=recentHit/recentN-baseline(tol),robust=robustEdgeWilson(hit,n,tol);
      const ra=firstHit/mid,rb=second.length?secondHit/second.length:0;
      const stability20=clamp(1-Math.abs(edge-recentEdge)/Math.max(0.05,Math.abs(edge)+0.02),0,1);
      const stabilityHalf=clamp(1-Math.abs(ra-rb)/Math.max(0.10,Math.abs(ra)+Math.abs(rb)+0.10),0,1);
      const q={n,hit,edge,recentEdge,robustEdge:robust,stability:0.55*stability20+0.45*stabilityHalf};
      const target=targetWeight(q);w=seen?(w*(1-alpha)+target*alpha):target;seen=true;
    }
    weights[f]=clamp(seen?w:1,0.75,1.35);
  });
  return {perf,weights,meta:metaBacktest(h,tol)};
}
function familyEvidenceQuality(h,f,d){
  const key=hashHistory(h,0)+'|evidenceQuality15.9|'+f;
  cache.familyEvidenceQuality??=new Map();
  if(cache.familyEvidenceQuality.has(key))return cache.familyEvidenceQuality.get(key);
  const cs=candidates(h,f),best=cs[0],maxRel=Math.max(...d)*37,entropy=d.reduce((s,v)=>v>0?s-v*Math.log(v):s,0),entropyNorm=entropy/Math.log(37),concentration=clamp((maxRel-1)/2.5,0,1);
  const patternCount=clamp(cs.length/4,0,1),bestLen=best?clamp((best.len-2)/6,0,1):0,bestOcc=best?clamp(best.occ/6,0,1):0,bestRecency=best?0.55+0.45*((best.lastIndex+1)/Math.max(1,h.length)):0;
  const quality=clamp(0.30*concentration+0.20*(1-entropyNorm)+0.15*patternCount+0.15*bestLen+0.10*bestOcc+0.10*bestRecency,0,1);
  const result={quality,concentration,patternCount,bestLen,bestOcc,bestRecency,maxRelative:maxRel};cache.familyEvidenceQuality.set(key,result);return result;
}
function calibrateRelative(scores){
  const floor=0.015,temps=scores.map(v=>Math.sqrt(Math.max(v,0))),sum=temps.reduce((a,b)=>a+b,0)||1,base=temps.map(v=>v/sum),floored=base.map(v=>Math.max(floor,v)),total=floored.reduce((a,b)=>a+b,0)||1;return floored.map(v=>v/total);
}
function model(h,tol){
  ...
  const a=adaptive(h,tol),familyScores={},familyTargets={};FAMILIES.forEach(f=>{familyScores[f]=familyDistribution(h,f);familyTargets[f]=familyTarget(h,f)});
  const dirDist=directionDistribution(h),dirPattern=directionPatternDistribution(h),score=Array(37).fill(0),support=Array(37).fill(0),contrib={},evidenceQuality={};
  const active=FAMILIES.filter(f=>familyScores[f].some(v=>v>0)).length;if(active<2){cache.model.set(key,null);return null}
  const familyWeightSum=FAMILIES.reduce((s,f)=>s+(a.weights[f]||1),0)||1;
  for(const f of FAMILIES){
    const d=familyScores[f],perf=a.perf[f],eq=familyEvidenceQuality(h,f,d);
    const target=familyTargets[f];
    const peers=FAMILIES.filter(g=>g!==f&&familyTargets[g]!=null);
    const agreementPeers=peers.length&&target!=null?peers.filter(g=>familyTargets[g]===target).length/peers.length:0;
    eq.agreement=agreementPeers;
    // Historical weight determines how much we trust the family in general;
    // current evidence quality determines how much of that weight is used now.
    const historicalW=(a.weights[f]||1)/familyWeightSum*FAMILIES.length;
    const sampleConf=perf.n?perf.n/(perf.n+50):0.15;
    const currentQuality=0.80*eq.quality+0.20*agreementPeers;
    const currentInfluence=0.55+0.45*currentQuality;
    const performanceConfidence=0.70+0.30*clamp(sampleConf,0,1);
    const fw=historicalW*currentInfluence*performanceConfidence;
    contrib[f]=Array(37).fill(0);
    for(let n=0;n<37;n++){
      const evidence=Math.max(0.000001,d[n]*37),excess=Math.max(0,evidence-1),term=Math.log1p(fw*excess);
      score[n]+=term;contrib[f][n]=term;support[n]+=term;
    }
  }
  // Direction remains contextual: it can reinforce a candidate already supported by
  // the family ensemble, but it is not an independent sixth family.
  for(let n=0;n<37;n++){
    const dirEvidence=Math.max(0.000001,(0.62*dirDist[n]+0.38*dirPattern[n])*37);
    score[n]+=0.42*Math.log1p(Math.max(0,dirEvidence-1));
  }
  // Cross-feature agreement rewards independent evidence, not a repeated historical
  // successor. No single family receives a special direct-transition bonus.
  const pairMix=[['sequence','jump'],['sequence','transition'],['jump','joint'],['jump','transition'],['pair','transition']];
  pairMix.forEach(([fa,fb])=>{
    const da=familyScores[fa],db=familyScores[fb];
    for(let n=0;n<37;n++){
      const aev=Math.max(0.000001,da[n]*37),bev=Math.max(0.000001,db[n]*37);
      const agreement=Math.sqrt(Math.max(0,aev-1)*Math.max(0,bev-1));score[n]+=0.075*Math.log1p(agreement);
    }
  });
  const last=h.at(-1);let cw=0,ccw=0;for(let n=0;n<37;n++){const j=jmp(last,n);if(j>0)cw+=dirDist[n];else if(j<0)ccw+=dirDist[n]}
  const dirSign=cw>=ccw?1:-1;for(let n=0;n<37;n++){const j=jmp(last,n);if(j!==0&&Math.sign(j)===dirSign)score[n]+=0.055*Math.min(2,Math.abs(j)/6)}
  // Mild recency penalty only for unusually repeated exact numbers; it cannot create a target.
  const recentNums=h.slice(-8),cnt=new Map();recentNums.forEach(n=>cnt.set(n,(cnt.get(n)||0)+1));for(let n=0;n<37;n++){const c=cnt.get(n)||0;if(c>=3)score[n]*=(c===3?.96:c===4?.91:.86)}
  const min=Math.min(...score);for(let n=0;n<37;n++)score[n]-=min;
  const total=score.reduce((a,b)=>a+b,0)||1,raw=score.map(v=>v/total),probs=calibrateRelative(raw),ranked=raw.map((v,n)=>({n,v,p:probs[n],support:support[n]})).sort((x,y)=>y.p-x.p||y.support-x.support),top=ranked[0],second=ranked[1],lead=top.p-second.p;
  const mean=1/37,topLift=top.p/mean;
  // // No arbitrary probability floor is used to reject a ranked ensemble prediction.
  if(!top||topLift<1.18){cache.model.set(key,null);return null}
  const zoneProbability=ranked.filter(x=>dist(x.n,top.n)<=tol).reduce((s,x)=>s+x.p,0),edge=zoneProbability-baseline(tol),familyRobust=FAMILIES.map(f=>a.perf[f].n?a.perf[f].robustEdge:0).filter(Number.isFinite),avgRobust=familyRobust.length?familyRobust.reduce((s,x)=>s+x,0)/familyRobust.length:0,familyRecent=FAMILIES.map(f=>a.perf[f].n?a.perf[f].recentEdge:0).filter(Number.isFinite),avgRecent=familyRecent.length?familyRecent.reduce((s,x)=>s+x,0)/familyRecent.length:0;
  const consensusCount=FAMILIES.filter(f=>a.perf[f].n&&familyTargets[f]===top.n).length,consensus=active?consensusCount/active:0,stability=clamp(0.55*clamp(1-Math.abs(avgRobust-avgRecent)/Math.max(0.05,Math.abs(avgRobust)+0.02),0,1)+0.45*(FAMILIES.reduce((s,f)=>s+(a.perf[f].n?a.perf[f].stability:0),0)/Math.max(1,FAMILIES.filter(f=>a.perf[f].n).length)),0,1),recentQuality=clamp((avgRecent+baseline(tol))/Math.max(0.001,1-baseline(tol)),0,1);
  const agreementCount=FAMILIES.filter(f=>familyScores[f][top.n]>=1/37*1.10).length,agreement=agreementCount/FAMILIES.length;
  const confidence=Math.round(clamp(25+lead*1000+agreement*24+consensus*16+stability*14+clamp(avgRobust*300,-5,18),5,99));
  const quality=Math.round(clamp(agreement*28+consensus*17+stability*20+clamp((avgRobust+0.03)/0.08,0,1)*20+recentQuality*15,0,100));
  const signal=confidence>=72&&avgRobust>=0.008&&quality>=70?'HIGH':'LOW';
  const target=top.n,jump=jmp(last,target),familyContrib=FAMILIES.map(f=>({family:f,score:contrib[f][target]||0,weight:a.weights[f]||1,evidenceQuality:evidenceQuality[f]?.quality||0,agreement:evidenceQuality[f]?.agreement||0})).sort((a,b)=>b.score-a.score);
  const result={target,prob:top.p,zoneProbability,ranking:ranked.slice(0,12),predDir:cw>=ccw?'CW':'CCW',cw,ccw,adaptive:a,seq:candidates(h,'sequence').slice(0,4),joint:candidates(h,'joint').slice(0,3),jumps:candidates(h,'jump').slice(0,3),edge,robustEdge:avgRobust,confidence,signal,lead,avgEdge:avgRobust,avgRecent,stability,consensusCount,activeModels:active,jump,quality,familyTargets,directionContext:directionContext(h),directionPattern:dirPattern,familyContributions:familyContrib,agreementCount,coverage:topLift,evidenceQuality};
